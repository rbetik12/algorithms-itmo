...
Вам даны два массива,  и , содержащие соответственно  и  элементов. Числа, которые нужно будет отсортировать, имеют вид...
 Иными словами, каждый элемент первого массива нужно умножить на каждый элемент второго массива.

Пусть из этих чисел получится отсортированная последовательность  длиной . Выведите сумму каждого десятого элемента
...
void int_radix_sort(int[], unsigned int);

int main() {
    int n, m = 0;
    io >> n >> m;
    const unsigned int size = n * m;
    int* const firstArray = new int[n];
    int* const combinedArray = new int[size];
    for (int i = 0; i < n; i++) {
        io >> firstArray[i];
    }
    int max = -1;
    for (int i = 0; i < m; i++) {
        ...
        io >> secondArrEl;
        for (int j = 0; j < n; j++) {
            int res = firstArray[j] * secondArrEl;
            if (res > max)
                max = res;
            combinedArray[j + i * n] = res;
        }
    }
    int_radix_sort(combinedArray, size);
    long long int sum = 0;
 ...
 */
void int_radix_sort(int vector[], unsigned int size) {
